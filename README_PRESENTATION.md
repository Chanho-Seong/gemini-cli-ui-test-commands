...
- 병렬처리에 따른 매우 많은 컴퓨팅 자원을 필요로 함에 주의
...
/agents:pipeline  (또는 bin/run-pipeline.sh)
  ...
  └─────────────┘     │ + Run     │     └──────────┘     └───────────┘     └──────────┘     └──────────┘
   device-pool.sh      └───────────┘      aggregate-       verifier-agent    coder-agent      pr-agent
   discover             create-test-       test-results.py  (mobile-mcp)      (병렬, 클래스별)
                        tasks.py +         (API 미사용)
                        run-test-android.sh
                        (API 미사용, 병렬)
```
...
| 에이전트 | 역할 | 주요 출력 |
|----------|------|-----------|
| **run-test-android.sh** | APK 1회 빌드 → 모든 디바이스에 설치 → am instrument 샤딩 병렬 실행 (Gemini API 미사용) | `uitest_results.json` |
| **run-test-ios.sh** | xcodebuild test 실행 → 디바이스풀 연동 → 결과 JSON 생성 (Gemini API 미사용) | `uitest_results.json` |
| **verifier-agent** | 실패한 테스트를 mobile-mcp로 실제 기기에서 재현 → 에뮬레이터 전용 이슈 vs 실제 버그 분류 (다중 디바이스 병렬 검증) | `device_verification.json` |
...
**자동 연동**: `run-test-android.sh`와 `run-agent-with-retry.sh`가 테스트 실행/verifier-agent 실행 시 자동으로 acquire → 실행 → release. `trap`으로 비정상 종료 시에도 해제 보장.
...
| `/agents:run-test [옵션]` | Android/iOS 프로젝트 자동 감지 → 플랫폼별 테스트 스크립트 위임 실행 |
| `/agents:run-all [agent_type]` | 모든 pending 태스크 일괄 실행 (디바이스풀 제한 적용) |
| `/agents:verify <path>` | verifier-agent 태스크 생성 (다중 디바이스 병렬 검증, 실패 샤딩) |
...
│       ├── pipeline.toml   # 전체 파이프라인
│       ├── run-test.toml   # 플랫폼별 테스트 실행 (자동 감지)
│       ├── run-all.toml    # 일괄 실행
│       ├── run.toml        # 단일 태스크 실행
│       ├── verify.toml     # 다중 디바이스 병렬 검증
│       ├── fix.toml        # 코드 수정
│       ├── pr.toml         # PR 생성
│       ├── start.toml      # 태스크 생성
│       ├── status.toml     # 상태 조회
│       └── type.toml       # 에이전트 목록
├── extensions/
│   ├── base-orchestrator/  # 파이프라인 오케스트레이션 로직
...
│   └── tdd-agent/          # TDD 워크플로우
├── hooks/
│   ├── notify-stage.sh         # 파이프라인 단계 전환 시 macOS 알림
│   └── notify-pipeline-done.sh # 파이프라인 완료 시 macOS 알림
├── rules/
│   ├── android-uitest-conventions.md   # Android UITest 컨벤션
│   ├── ios-uitest-conventions.md       # iOS UITest 컨벤션
│   └── json-output-formatting.md       # JSON 출력 스키마 규칙
bin/
├── run-pipeline.sh                 # E2E 파이프라인 오케스트레이터
├── device-pool.sh                  # 디바이스 풀 매니저
├── create-test-tasks.py            # 스위트/클래스/패턴 → 태스크 JSON 생성 (API 미사용)
├── run-test-android.sh             # Android UI 테스트 직접 실행 (API 미사용)
├── run-test-ios.sh                 # iOS UI 테스트 직접 실행 (API 미사용)
├── parse-am-instrument-results.py  # am instrument 출력 → uitest_results.json
├── split-failures.py               # 실패 테스트 라운드 로빈 샤딩 (다중 디바이스 검증용)
├── merge-verification-results.py   # 다중 디바이스 검증 결과 병합
├── run-agent-with-retry.sh         # 모델 재시도 + 디바이스풀 연동 (verifier/coder/pr용)
├── reconcile-tasks.sh              # 태스크 상태 정합 + 디바이스 잠금 정리
├── reset-tasks.sh                  # 태스크 초기화/정리 (프로세스 종료, 로그/잠금 삭제)
├── parse-android-test-results.py   # JUnit XML → uitest_results.json
├── log-utils.sh                    # 공유 로깅 함수 (다른 스크립트에서 source)
...
| `/agents:run-test [옵션]` | Android/iOS 자동 감지 → 플랫폼별 테스트 실행 위임 | 테스트 |
| `/agents:run-all [agent_type]` | 모든 pending 태스크 일괄 실행 (디바이스풀 제한) | 실행 |
| `/agents:run [task_id]` | 단일 태스크 실행 | 실행 |
| `/agents:verify <path>` | verifier-agent 태스크 생성 (다중 디바이스 병렬 검증) | 검증 |
...
# 2. 테스트 태스크 생성 (Gemini API 미사용)
python3 bin/create-test-tasks.py

# 3. 테스트 실행 (디바이스 수만큼 병렬, Gemini API 미사용)
# 파이프라인에서는 run-test-android.sh를 자동으로 호출합니다
# 수동 실행 시: bin/run-test-android.sh --variant googleBeta --module yogiyo --class <testClassFqn>
...
- **모든 에이전트**: pro → flash → 2.5-pro → 2.5-flash (용량 오류 시 순차 재시도)
- 참고: 테스트 실행(Stage 1~4)은 셸 스크립트로 직접 수행되므로 Gemini API를 사용하지 않습니다. 이 재시도 전략은 verifier/coder/pr-agent(Stage 5~7)에만 적용됩니다.

### 3.7 Gemini CLI 익스텐션

Gemini CLI 익스텐션은 프로젝트별로 AI의 역할, 도구, 설정을 확장하는 메커니즘입니다. 이 프로젝트의 서브 에이전트(verifier-agent, coder-agent, pr-agent 등)는 익스텐션으로 정의됩니다. 테스트 실행은 셸 스크립트(`run-test-android.sh`)로 직접 수행하여 API 토큰을 절약합니다.
...
- 예: `gemini -e verifier-agent -y -p "..."` → verifier-agent 페르소나와 제약만 적용된 전용 인스턴스가 실행됩니다.
- 참고: 테스트 실행은 `run-test-android.sh`로 직접 수행되어 Gemini API 토큰을 소모하지 않습니다.
...
```
  discover     create-tasks    run-tests     aggregate     verify        fix           pr
  ──────── ──▶ ──────────── ──▶ ──────── ──▶ ────────── ──▶ ──────── ──▶ ──────── ──▶ ────────
  디바이스      create-test-    run-test-     결과 병합     실기기 검증   코드 수정     PR 생성
  풀 초기화     tasks.py        direct.sh     (실패 집계)   (mobile-mcp)  (병렬, 커밋)  (gh CLI)
               (API 미사용)    (API 미사용)   (API 미사용)  [AI 에이전트]  [AI 에이전트]  [AI 에이전트]
                                                   │
                                              실패 없음 → 종료
                                              실패 있음 → 다음 단계
```
...
| Test Planning | ✅ 구현 | `bin/create-test-tasks.py` | 스위트 → 클래스별 태스크 (API 미사용) |
| Test Execution | ✅ 구현 | `bin/run-test-android.sh` | 빌드 1회 + am instrument 샤딩 실행 (API 미사용) |
| Result Check | ✅ 구현 | `run-pipeline.sh` (stage_aggregate) | 결과 파일 확인 + 실패 여부 판단 |
...
### 3.10 라이프사이클 훅 (Lifecycle Hooks)

`.gemini/hooks/` 디렉터리에 설정된 훅으로, 파이프라인 진행 상황을 macOS 데스크톱 알림으로 실시간 통지합니다.

| 훅 | 트리거 | 기능 |
|-----|--------|------|
| `notify-stage.sh` | AfterTool (`run_shell_command`) | 파이프라인 단계 전환 감지 (test → verify → fix → pr) → macOS 알림 ("Glass" 사운드) |
| `notify-pipeline-done.sh` | AfterAgent | 파이프라인 완료 시 최종 알림 ("Hero" 사운드) |

**감지 키워드**: 테스트 완료, 디바이스 검증, 코드 수정, PR 생성, "파이프라인 완료", "Pipeline Complete" 등

`settings.json`의 `hooks` 설정에서 타임아웃(5000ms)과 함께 등록됩니다.
